...
\todo{Move to chapter~\ref{chap:project}?}Each mind map is represented by its own, random UUID, so are all of its nodes, including root node.

\subsection{\todo{Should go to chapter~\ref{chap:project}?}Collaboration}
\label{subsec:collaboration}

Our application allows its users to edit maps collaboratively, either over the Internet in real time, or offline. {\em Offline} means that all changes introduced by fellow editors {\em offline} will get synchronized when particular person gets back on-line. \todo{We already know this...}There is a central server implemented using Akka.io which provides this functionality by means of RESTful services.
